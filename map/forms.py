...
class MarkerForm(forms.Form):
    marker_start = forms.ModelChoiceField(label="", initial=Marker.objects.get(id=6), queryset=Marker.objects.filter(type="start_dest"))
    marker_destination = forms.ModelChoiceField(label="", initial=Marker.objects.get(id=6), queryset=Marker.objects.filter(type="start_dest"))
