...
import random
from map.forms import ViewEstablishments
...
def index(request):
    ...
    view_estabs_form = ViewEstablishments()
    ...
    if request.method == "POST":
        ...
        view_estabs_form = ViewEstablishments(request.POST)
        all_routes = []

        if "estab_types" in request.POST.keys():
            all_routes.append(request.POST["estab_types"])
        ...
        if "marker_start" and "marker_destination" in request.POST.keys():
            ...
            marker_est_start = Marker.objects.filter(id=marker_start).values_list(
                "latitude", "longitude"
            )
            marker_est_dest = Marker.objects.filter(id=marker_destination).values_list(
                "latitude", "longitude"
            )

            intersections = Marker.objects.filter(type="intersection").values_list(
                "id", "latitude", "longitude"
            )
            marker_intersec_start = get_nearest_point(
                marker_est_start[0], intersections
            )
            marker_intersec_dest = get_nearest_point(marker_est_dest[0], intersections)

            routes = iligangraph(
                nx.DiGraph(), marker_intersec_start, marker_intersec_dest
            )
            for route in routes:
                ...
                route_with_estab = []
                preserved = Case(
                    *[When(pk=pk, then=pos) for pos, pk in enumerate(route[0])]
                )
                route_inter_markers = (
                    Marker.objects.filter(type="intersection", pk__in=route[0])
                ...
                route_estab = Marker.objects.filter(
                    type__in=["establishment", "start_dest"], pk__in=route[1]
                ).values("id", "latitude", "longitude", "name", "category_level", "type")
            ...
            try:
                ...
                try:
                    ...
                    messages.add_message(
                        request,
                        messages.INFO,
                        "ERROR: Imported data contain empty rows.",
                    )
    ...
    context["view_estabs_form"] = view_estabs_form
...
def modifymarker_view(request):
    if request.method == "POST":
        ...
        if modifyType == "" or name == "":
            return render(
                request, "error.html", {"error_name": "No means no! You can't do that."}
            )
        ...
        if modifyType == "add":
            if marker != None:
                return render(
                    ...
                    "error.html",
                    {
                        "error_name": "Marker Name already exist, check your available markers before attempting to add."
                    },
                )
            ...
            if isANumber:
                ...
                return render(
                    request,
                    "error.html",
                    {"error_name": "Invalid value for Longitude and Latitude"},
                )
            ...
            if marker == None:
                return render(
                    ...
                    "error.html",
                    {
                        "error_name": "Marker Name doesn't exist, check your available markers before attempting to remove."
                    },
                )
            else:
            ...
            return render(
                request,
                "error.html",
                {"error_name": "Oops! You finally found me. - Error"},
            )

        return redirect("/map")
    else:
        return render(
            request, "error.html", {"error_name": "Oops! You finally found me. - Error"}
        )
...
def nodes_within_radius_along_path(graph, node1, node2, radius=1.5):
    ...
    for node in graph.nodes(data=True):
        if "pos" in node[1].keys():
            ...
            if (min(x1, x2) <= node_x <= max(x1, x2)) and (
                min(y1, y2) <= node_y <= max(y1, y2)
            ):
...
def iligangraph(graph, start, end):
    ...
    graph.add_edge(
        416, 450, weight=100
    )  # possible to be removed. to recheck(added temporary weight = 0)
    ...
    graph.add_edge(
        341, 340, weight=98
    )  # these two line are possible to be removed. to recheck(added temporary weight = 0)
    ...
    for route in routes:
        ...
        for node in range(len(route)):
            try:
                get_establishment = nodes_within_radius_along_path(
                    graph, route[node], route[node + 1]
                )
        ...
        djik_estab = [
            estab for edge_estab in edge_establishments for estab in edge_estab
        ]
